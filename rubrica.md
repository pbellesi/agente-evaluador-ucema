...
**4.1.1 Qué se evalúa**
Que el trabajo sea un sistema agéntico real —no un chatbot ni un prompt suelto— con objetivo definido, contrato escrito, al menos una herramienta o conector real, salida estructurada y puntos de supervisión humana explícitos con el vocabulario del curso L0–L4.
...
- Descripción explícita de supervisión humana en términos L0–L4. `trabajo_final.pdf` define textualmente qué debe cubrir ese vocabulario: "qué hace solo, qué revisa una persona, quién firma". La evidencia debe mostrar las tres cosas para el sistema concreto: qué hace sin intervención, qué revisa una persona, y quién firma el resultado final.
...
1. **"Las seis piezas" del contrato.** `trabajo_final.pdf` menciona un "contrato escrito (system prompt + user prompt, con las seis piezas)" pero, tras revisar la totalidad de `00_fuentes/` disponible para este rol (`consigna/parcial_agente_evaluador.pdf`, `consigna/trabajo_final.pdf`, `docente/prompts_github.html`, y los tres documentos de `equipo/`), no se encontró ninguna definición de cuáles son esas seis piezas. La Dimensión 1 evalúa presencia y coherencia del contrato sin exigir una lista cerrada de seis piezas, para no inventar un requisito no respaldado. Si el material de clase (no incluido en este repositorio) define esas seis piezas explícitamente, la Dimensión 1 debería actualizarse para verificarlas una por una.
2. **Límite exacto entre los niveles L0–L4 (ambigüedad acotada).** `trabajo_final.pdf` sí define qué debe expresar el vocabulario L0–L4: "qué hace solo, qué revisa una persona, quién firma" — esa parte ya está incorporada como requisito respaldado en 4.1.2 y 4.5.2. Lo que ninguna fuente disponible (ambas consignas, `AGENTS.md`, `DECISIONES.md`, `00_fuentes/docente/`, `00_fuentes/equipo/`) define es el corte numérico exacto entre niveles (qué distingue a L0 de L1, a L1 de L2, y así sucesivamente). Por eso esta rúbrica exige que cada repositorio defina sus propios cortes L0–L4 específicos a su sistema, en lugar de imponer una tabla de niveles numérica no respaldada por ninguna fuente. Queda pendiente validar con material de clase si existe una definición estándar de esos cortes.
3. **Escala numérica de puntaje por dimensión.** La consigna oficial da los pesos como porcentajes (30/25/15/15/15) pero no fija explícitamente un rango de puntos por dimensión. Esta rúbrica adopta la convención de puntuar cada dimensión de 0 al valor de su peso (de modo que la suma total sea sobre 100). Si el agente corrector requiere otra convención de puntaje, debe ajustarse aquí y no de forma implícita en el agente.

Una cuarta ambigüedad de la v1 inicial (variedad exigida entre las tres corridas) se identificó, se revisó contra la fuente y se cerró en esta revisión: no era una ambigüedad real, sino un requisito no respaldado que se retiró de la Dimensión 3 (ver 4.3.3 y `DECISIONES.md`).
